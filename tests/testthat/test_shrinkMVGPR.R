test_shrinkGPR <- function(args, eval_points = c(-2, 0, 2), log_pred = FALSE) {
  ...
  # Fit model
  res <- do.call(shrinkMVGPR, args)

  # Test model object
  expect_s3_class(res, "shrinkMVGPR")
  expect_true("shrinkMVGPR" %in% class(res))
  ...
  expect_named(moments, c("means", "K", "Omega"))
  ...
  names_posterior <- c("thetas", "tau", "sigma2", "tau_Om", "Omega")
  ...
  res3 <- do.call(shrinkMVGPR, args)

  # Check that res3 is a shrinkMVGPR object and has the expected structure
  expect_s3_class(res3, "shrinkMVGPR")
  expect_true("shrinkMVGPR" %in% class(res3))
