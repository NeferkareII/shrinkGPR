...
This repository contains the development version of `shrinkGPR` package for Gaussian Process Regression with hierarchical shrinkage priors. 
`shrinkGPR` uses the `torch` package to estimate the parameters via variational inference and does not function without it. Specifically, it is designed with GPU acceleration via CUDA in mind and may be slow without it. Guidance on how to install `torch` with CUDA support can be found [here](https://cran.r-project.org/web/packages/torch/vignettes/installation.html).
